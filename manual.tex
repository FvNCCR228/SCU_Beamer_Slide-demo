...
\subtitle{用户手册~~~~Rev~1.1a} % subtitle 未设置页脚显示项, 请在 title 中设置.
\author[我不卷, 你才卷]{\noindent 马老卷}
...
\begin{frame}[fragile]{宏包参数}
	...
	\quad\textcolor{HYgreen}{v1.1a(2021/12/30)}中, option \Arg{chinese}尚未修改, 请继续沿用此参数, 但其余所有参数均无法继续兼容.\\[2ex]
...
\begin{frame}[fragile]{宏包参数键值含义}
	\scriptsize
	\begin{multicols}{3}
		\textcolor{JXred}{\Arg{MathFont}}\hfill \textbf{数学字体}
		\begin{itemize}
			\item \Arg{LM}: 设置数学字体为Latin Modern Math.
			\item \Arg{XITS}: 设置数学字体为XITS Math.
		\end{itemize}
		\textcolor{JXred}{\Arg{BlockDisplay}}\hfill \textbf{区块颜色显示}
		\begin{itemize}
			\item \Arg{colorful}: 设置区块颜色为彩色.
			\item \Arg{allred}: 设置区块颜色均为锦秀红.
			\item \Arg{allgrey}: 设置区块颜色均为优雅灰.
		\end{itemize}
		\textcolor{JXred}{\Arg{CodeDisplay}}\hfill \textbf{代码高亮显示}
		\begin{itemize}
			\item \Arg{listing}: 设置代码高亮引擎为listing.
			\item \Arg{minted}: 设置代码高亮引擎为minted. (推荐, 但设置前请阅读手册对应部分)
		\end{itemize}
		\textcolor{JXred}{\Arg{ContentMuticols}}\hfill \textbf{目录帧双栏显示}
		\begin{itemize}
			\item \Arg{true}: 是.
			\item \Arg{false}: 否.
		\end{itemize}
		\textcolor{JXred}{\Arg{Background}}\hfill \textbf{背景显示}
		\begin{itemize}
			\item \Arg{true}: 是.
			\item \Arg{false}: 否.
		\end{itemize}
...
\begin{frame}{~}
	...
	\end{columns}
	\centering\vskip4ex
	SCU Beamer Slide-demo, Rev 1.1a
\end{frame}
