...
% Version: 2022/03/16 v1.3a (Original Version: 2021/11/30 v1.0a).
...
% ----------------
\usetheme[chinese, MathFont=LM,%
	BlockDisplay=colorful, CodeDisplay=minted,%
	ColorDisplay=JXred, ContentMuticols=ture,%
	Background=true]{scu}
...
% ----------------
\title[四川大学Beamer模板 | User's Manual]{\LARGE 四川大学Beamer模板}
\subtitle{用户手册~~~~Rev~v1.3a} % subtitle 未设置页脚显示项, 请在 title 中设置.
\author[我不卷, 你才卷]{\noindent 马老卷}
\institute{%
...
\begin{frame}{Info.}
	...
	\begin{multicols}{2}
		\begin{itemize}
			\item Establish: \textcolor{scugreen}{v1.3a (2022/03/16)}
		\end{itemize}
	\end{multicols}
	\vspace{2ex}本小节摘自\textcolor{scugreen}{Beamer User Guide}, 详细内容请在终端输入\alert{\texttt{texdoc beamer}}查看完整手册.
\end{frame}

\begin{frame}{文档类部分参数}{draft, t\&c\&b, aspectratio}
	\textbf{调用命令}: \cmd{documentclass}\oarg*{\Arg{key1}=\Arg{value1},\ldots}\marg*{beamer}\par\vspace{1ex}
	...
	\alert{\texttt{aspectratio} = \Arg{value}}\hfill \textbf{Slide比例}
	\begin{itemize}
...
\begin{frame}{Info.}
	...
	\begin{multicols}{2}
		\begin{itemize}
			\item Establish: \textcolor{scugreen}{v1.1a (2021/12/30)}
...
\begin{frame}{宏包参数键值对照}
	\textbf{调用命令}: \cmd{usetheme}\oarg*{\Arg{key1}=\Arg{value1},\ldots}\marg*{scu}\par\vspace{1ex}
	\begin{table}[h]
		\centering
		\caption{可选参数键值对照}
		\begin{tabular}{>{\raggedleft\arraybackslash}p{0.275\textwidth}
			>{\raggedleft\arraybackslash}p{0.275\textwidth}>{\raggedleft\arraybackslash}p{0.275\textwidth}}
			version \alert{\Arg{key}}              & \multicolumn{2}{l}{`C' means the key here supports a custom value.}          \\
			\Arg{value}                            & \multicolumn{2}{l}{`D' means the value here is the default one.}             \\ \hline
			v1.1a \alert{\texttt{MathFont}}        & v1.3a \alert{\texttt{BlockDisplay}} & v1.1a \alert{\texttt{CodeDisplay}}     \\
			(D) \texttt{LM}                        & (D) \texttt{colorful}               & (D) \texttt{listing}                   \\
			\texttt{XITS}                          & \texttt{followtheme}                & \texttt{minted}                        \\
			                                       & \texttt{allgrey}                    &                                        \\ \hline
			v1.3a (C) \alert{\texttt{MintedStyle}} & v1.3 \alert{\texttt{ColorDisplay}}  & v1.1a \alert{\texttt{ContentMuticols}} \\
			(D) \texttt{lightmode}                 & (D) \texttt{JXred}                  & (D) \texttt{true}                      \\
			\texttt{darkmode}                      & \texttt{BSblue}                     & \texttt{false}                         \\ \hline
			v1.1a \alert{\texttt{Background}}      &                                     &                                        \\
			(D) \texttt{true}                      &                                     &                                        \\
			\texttt{false}                         &                                     &
		\end{tabular}
	\end{table}
...
\begin{frame}{宏包参数键值含义}
	\begin{multicols}{3}
		\alert{\texttt{MathFont}}\hfill \textbf{数学字体设置}
		\begin{itemize}
			\item \texttt{LM}: Latin Modern Math字体.
			\item \texttt{XITS}: XITS Math字体.
		\end{itemize}
		
		\alert{\texttt{BlockDisplay}}\hfill \textbf{区块颜色显示}
		\begin{itemize}
			\item \texttt{colorful}: 彩色模式.
			\item \texttt{followtheme}: 跟随主题.
			\item \texttt{allgrey}: 灰色模式.
		\end{itemize}
		
		\alert{\texttt{CodeDisplay}}\hfill \textbf{代码高亮显示}
		\begin{itemize}
			\item \texttt{listing}: listing引擎.
			\item \texttt{minted}: minted引擎. (推荐, 设置前请阅读手册对应部分)
		\end{itemize}
		
		\alert{\texttt{MintedStyle}}\hfill \textbf{minted样式}\\
		\textbf{需加载minted引擎}
		\begin{itemize}
			\item \texttt{lightmode}: 亮色模式, Pygments default样式.
			\item \texttt{darkmode}: 暗色模式, Pygments rrt样式.
			\item \Arg{custom}: 自定义. (设置前请阅读手册对应部分)
		\end{itemize}
		
		\alert{\texttt{ColorDisplay}}\hfill \textbf{主题色显示}
		\begin{itemize}
			\item \texttt{JXred}: 锦锈红色调.
			\item \texttt{BSblue}: 宝石蓝色调.
		\end{itemize}
		
		\alert{\texttt{ContentMuticols}}\hfill \textbf{目录帧双栏显示}
		\begin{itemize}
			\item \texttt{true}: 是.
			\item \texttt{false}: 否.
		\end{itemize}
		
		\alert{\texttt{Background}}\hfill \textbf{背景显示}
		\begin{itemize}
			\item \texttt{true}: 是.
			\item \texttt{false}: 否.
		\end{itemize}
...
\begin{frame}{字号}
	...
	\begin{columns}
		...
		\begin{column}{.22\textwidth}
			老王, 形翼门自称超级英雄第二, 道上人称Neighbor, 其特异能力是形态的缩放, 为响应他第二的自称, 特将其置于第三节\\[.5ex]
			在Beamer中normalsize是默认字号. 非特殊情况时, 为保证演示效果请不要改变全局默认字号. 当部分文字有修改字号的需求时, 一般将范围设置为footnotesize - Large.
		\end{column}
	\end{columns}
\end{frame}
...
\cprotEnv\begin{frame}
	...
	\begin{scucode}[comment={%
		...
		listing above comment]{过长环境切割}[GuochangHjqg]{tex}
...
\begin{frame}
	...
	\end{columns}
	\centering\vskip4ex
	SCU Beamer Slide-demo, Rev 1.3a
\end{frame}
